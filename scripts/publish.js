...
if (version) {
  const execVersionTask = () => execAsync(`npm version ${version} -m "chore: version %s"`, 'execVersionTask');
  ...
  const execCopyTask = () => {
    ...
    shell.cp('.prettierrc.js', 'dist');
  };
  const execNpmConfigTask = () => execAsync('npm config get registry', 'execNpmConfigTask');
  const execPublishTask = () => execAsync(`npm publish --access=public dist --tag ${tag}`, 'npmRegistry');
  const execSyncTaoBaoTask = () =>
    execAsync('curl -X PUT https://npm.taobao.org/sync/eslint-config-kzf', 'execSyncTaoBaoTask');
